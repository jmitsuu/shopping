...
import { StarIcon, Square3Stack3DIcon, ShoppingCartIcon } from "@heroicons/vue/24/solid";
...
<template>
    <section class="min-h-screen  py-10 flex flex-col justify-center items-center bg-slate-50  ">
        <div class="mx-auto container  border-[0.1rem] rounded-md p-3  bg-white">
            ...
            <div class="xl:w-2/4 min-w-[800px] mx-auto justify-center  items-center border-[0.1rem] flx xl:flex  rounded-md mt-4 xl:pl-10 xl:pt-9 p-4"
                v-for="item in [store.infoProduct]" :key="item.id">
                ...
                <div class="xl:ml-10 w-full">
                    ...
                    <div class="flex gap-4 relative text-gray-600 w-full border-[0.1rem] border-gray-300 p-3 rounded-md">
                        <h2 class="border-r-2 pr-1 cursor-pointer">
                            <span class="font-bold flex justify-center items-center text-[1.1rem] mr-3 ">{{ item.rating.rate
                            }} <span class="text-xs ml-1 text-center">/5</span>
                                <div class="flex h-10 w-20 ml-4 items-center">
                                    <StarIcon class="h-6 w-6" :class="item.rating.rate >= 1
                                            ? ' text-yellow-600'
                                            : '  text-yellow-600/10'
                                        " />
                                    <StarIcon class="h-6 w-6" :class="item.rating.rate >= 2
                                        ? ' text-yellow-600'
                                        : '  text-yellow-600/10'
                                        " />
                                    <StarIcon class="h-6 w-6" :class="item.rating.rate >= 3
                                        ? ' text-yellow-600'
                                        : '  text-yellow-600/10'
                                        " />
                                    <StarIcon class="h-6 w-6" :class="item.rating.rate >= 4
                                        ? ' text-yellow-600'
                                        : '  text-yellow-600/10'
                                        " />
                                    <StarIcon class="h-6 w-6" :class="item.rating.rate >= 4.7
                                        ? ' text-yellow-600'
                                        : '  text-yellow-600/10'
                                        " />
                                </div>
                    ...
                    <div>
                        <h1 v-if="priceSplit = item.price / 8">
                            8x de <span class="text-red-700 font-semibold">{{ priceSplit.toFixed(2) }}</span> sem juros no
                            cartão
                        </h1>
                    ...
                    <div class="flex gap-8 text-sm mt-6 text-gray-700">
                        ...
                        <div class="relative h-full">
                            ...
                            <p @click="modal = !modal" class="bg-gray-50 flex p-3 cursor-pointer border-2">
                                ...
                                <span> {{ dataLocal.uf }}, {{ dataLocal.localidade }}, {{ dataLocal.logradouro }} </span>
                            </p>
                            <div v-if="modal" class="absolute xl:flex  gap-3 bg-gray-300 rounded-sm p-5">
                                <input placeholder="Digete um Cep" class="w-32 py-4 pl-2 rounded-sm outline-none"
                                    v-model="inputCep" required />
                                <button
            ...
            <div class="mx-auto min-w-[800px] xl:w-2/4 border-t-[0.1rem] p-3  bg-white">
                <div class=" mt-10 ">
                    <h1 class="uppercase font-bold text-gray-800"><span class=" border-b-2 pb-2">Avaliações</span></h1>

                    <div class="mt-10 " v-for="item in [store.infoProduct]" :key="item.id">
                        <h1 class="text-[4.1rem] font-bold text-gray-700">{{ item.rating.rate }}<span
                                class="text-xl">/5</span></h1>
                        <div class="flex">
                            <StarIcon class="h-6 w-6" :class="item.rating.rate >= 1
                                ? ' text-yellow-600'
                                : '  text-yellow-600/10'
                    ...
                    <div class="mt-10">
                        <h1 class="uppercase font-bold text-gray-800 border-b-2 pb-5 text-[1.3rem] ">Avaliações mais recentes</h1>
                        <div class="flex flex-col mt-6 gap-10">
                            ...
                            <button class="p-4 border-2 w-44 mx-auto text-gray-700 bg-gray-50 rounded-sm hover:bg-gray-400 hover:text-white uppercase">Carregar Mais</button>
